...
export default function CheckoutPage() {
  ...
  return (
    <div className="bg-surface min-h-screen py-10 px-4 transition-colors duration-300">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6 bg-surface-elevated p-6 rounded shadow dark:shadow-[0_2px_8px_rgba(0,0,0,0.3)] border border-transparent dark:border-border-custom transition-colors duration-300">
          <h2 className="text-xl font-semibold text-text-primary">Dirección de Envío</h2>
          <div className="flex gap-4">
            <div className="w-1/2">
              <input
                ...
                type="text"
                placeholder="Nombre"
                className="w-full p-3 border border-input-border rounded bg-input-bg text-text-primary placeholder:text-text-muted transition-colors duration-300"
              />
              {fieldErrors.nombre && (
            ...
            <div className="w-1/2">
              ...
              {fieldErrors.apellidos && (
                <p className="text-red-500 text-sm mt-1">{fieldErrors.apellidos}</p>
              )}
            </div>
          </div>
          <div className="w-1/2">
          ...
          <div className="flex gap-4">
            <div className="w-1/2">
              <select
                ...
                className="w-full p-3 border border-input-border rounded bg-input-bg text-text-primary transition-colors duration-300"
              >
            ...
            <div className="w-1/2">
              ...
              {fieldErrors.canton && (
                <p className="text-red-500 text-sm mt-1">{fieldErrors.canton}</p>
              )}
            </div>
          </div>
          <div className="flex gap-4">
          ...
          <div className="border border-border-custom p-4 rounded space-y-3 bg-surface mt-6 transition-colors duration-300">
            <h3 className="text-lg font-bold text-text-primary">Pago</h3>
            <p className="text-sm text-text-muted">
              Todas las transacciones son seguras y están encriptadas.
            </p>

            {/* Número de tarjeta */}
            <label className="block font-medium text-sm text-text-primary">Número de tarjeta</label>
            <div className="relative">
              <input
                ...
                className="w-full p-3 border border-input-border rounded pr-16 bg-input-bg text-text-primary placeholder:text-text-muted transition-colors duration-300"
              />
              {cardType && (
                <div className="absolute top-1/2 right-3 -translate-y-1/2">
                  <Image src={`/icons/${cardType}.svg`} alt={cardType} width={48} height={30} />
                </div>
              )}
            </div>
          ...
          <button
            ...
            className="bg-text-primary text-background w-full py-3 rounded font-semibold mt-6 hover:opacity-90 transition-all duration-200"
          >
            Pagar ahora
          </button>
        ...
        {/* Resumen */}
        <div className="bg-surface-elevated p-6 rounded shadow-lg dark:shadow-[0_4px_12px_rgba(0,0,0,0.4)] sticky top-10 max-h-[90vh] overflow-y-auto border border-transparent dark:border-border-custom transition-colors duration-300">
          <h2 className="text-xl font-bold mb-4 text-text-primary">Resumen</h2>
          <div className="space-y-4">
            {cartItems.map((item, index) => (
              <div key={index} className="flex gap-4 border-b border-border-custom pb-4">
                <img
                  src={item.imageUrl}
                  alt={item.name}
                ...
                <div className="flex-1">
                  ...
                  {item.selectedSize && (
                    <p className="text-sm text-text-muted">Talla {item.selectedSize}</p>
                  )}
                  {item.selectedColor && (
        ...
        {toastType && <Toast show={showToast} type={toastType} message={toastMessage} />}
      </div>
    </div>
  );
}
