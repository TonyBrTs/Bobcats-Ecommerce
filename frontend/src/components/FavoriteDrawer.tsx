    ...
    <>
      ...
      >
        <div className="flex justify-between items-center p-4 border-b border-border-custom">
          <h2 className="text-lg font-semibold text-text-primary">Tus favoritos</h2>
        ...
        <div className="p-4 space-y-6 overflow-y-auto h-[calc(100%-4rem)]">
          {!user ? (
            <p className="text-sm text-text-muted">Debes iniciar sesión para ver tus favoritos.</p>
          ) : favoriteItems.length === 0 ? (
            <p className="text-sm text-text-muted">No tienes productos en favoritos todavía.</p>
          ) : (
              ...
              {favoriteItems.map((item, index) => (
                <li key={index} className="flex gap-4 border-b border-border-custom pb-4 relative">
                  ...
                  <button
                    ...
                    className="absolute top-0 right-0 p-2 text-text-muted hover:text-red-500"
                  >
